...
3. 已补充 Windows 静默启动脚本，当前可通过 `Init_Scripts/start_frontend_silent.ps1` 和 `stop_frontend.ps1` 无黑框启动或停止前端。
4. 已修正静默启动脚本的项目根目录解析与隐藏窗口启动方式，当前可同时拉起 8501 前端端口和 8765 controller 流接口。
...
4. 启动链路已验证可在静默模式下同时带起 Streamlit 前端与 controller 内部流服务。
