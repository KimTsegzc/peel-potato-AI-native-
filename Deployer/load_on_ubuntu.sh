...
GITHUB_HTTPS_MIRROR="${GITHUB_HTTPS_MIRROR:-https://hub.fastgit.xyz/}"
...
configure_git_mirror() {
  if [[ -z "$GITHUB_HTTPS_MIRROR" ]]; then
    return 0
  fi

  git config --global url."$GITHUB_HTTPS_MIRROR".insteadOf "https://github.com/"
  su - "$APP_USER" -c "git config --global url.'$GITHUB_HTTPS_MIRROR'.insteadOf 'https://github.com/'"
}
...
log_info "GITHUB_HTTPS_MIRROR=$GITHUB_HTTPS_MIRROR"
