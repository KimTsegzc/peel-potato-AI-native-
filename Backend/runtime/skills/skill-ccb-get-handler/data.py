...
class HandlerChain:
    ...
    def render(self) -> str:
        department_head = self.department_head or "部门总未明确"
        supervising_head = self.supervising_head or "分管总未明确"
        owner_name = self.owner_name or "岗位负责人未明确"
        return f"{department_head}—{supervising_head}—{owner_name}"


def _format_chain_member(record: HandlerRecord | None, fallback_label: str, *, use_role_display: bool) -> str:
    if record is None:
...
class CCBHandlerTable:
    ...
    def resolve_chain(self, record: HandlerRecord) -> HandlerChain:
        ...
        return HandlerChain(
            department_head=_format_chain_member(department_head, "部门总未明确", use_role_display=False),
            supervising_head=_format_chain_member(supervising_head, "分管总未明确", use_role_display=False),
            owner_name=_format_chain_member(record, "岗位负责人未明确", use_role_display=True),
        )
